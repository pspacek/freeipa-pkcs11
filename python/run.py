...
if __name__ == '__main__':
    ...
    key = p11.find_keys(ipapkcs11.KEY_CLASS_PUBLIC_KEY, label=u"replica1", cka_wrap=True)[0]
    key_priv = p11.find_keys(ipapkcs11.KEY_CLASS_PRIVATE_KEY, label=u"replica1", cka_unwrap=True)[0]
    key2_priv = p11.find_keys(ipapkcs11.KEY_CLASS_PRIVATE_KEY, label=u"replica2", cka_unwrap=True)[0]
    key2 = p11.find_keys(ipapkcs11.KEY_CLASS_PUBLIC_KEY, label=u"replica2", cka_wrap=True)[0]
    log.debug('key handle: %s', key)

    test_list = p11.find_keys(ipapkcs11.KEY_CLASS_PUBLIC_KEY, label=u"replica666")
    assert len(test_list) == 0, "list should be empty because label replica666 should not exist"

    key3 = p11.find_keys(ipapkcs11.KEY_CLASS_SECRET_KEY, label=u"žžž-aest", id="m")[0]
    ...
    log.debug("get label: %s", p11.get_attribute(key, ipapkcs11.CKA_LABEL))
    try:
