...
void
wrap_private_key(CK_FUNCTION_LIST_PTR p11, CK_SESSION_HANDLE session, CK_OBJECT_HANDLE masterKey, CK_OBJECT_HANDLE key)
{
     CK_RV rv;
     ...
     p11->C_WrapKey(session, &wrappingMech, masterKey, key, NULL, &wrappedKeyLen);
     check_return_value(rv, "private key wrapping: get buffer length");
     pWrappedKey = malloc(wrappedKeyLen);
     if (pWrappedKey == NULL) {
	     rv = CKR_HOST_MEMORY;
	     check_return_value(rv, "private key wrapping: buffer allocation");
     }
     rv = p11->C_WrapKey(session, &wrappingMech, masterKey, key, pWrappedKey, &wrappedKeyLen);
     check_return_value(rv, "private key wrapping: real wrapping");
...
void
wrap_private_keys(CK_FUNCTION_LIST_PTR p11, CK_SESSION_HANDLE session)
{
     CK_RV rv;
     ...
     CK_ULONG objectCount;
     CK_OBJECT_HANDLE object;
     CK_OBJECT_HANDLE masterKey = NULL;

     masterKey = find_master_key(p11, session);
     ...
     while (objectCount > 0) {
          wrap_private_key(p11, session, masterKey, object);
